    ...
    <ThemeProvider theme={defaultTheme}>
      <Box sx={{ display: 'flex' }}>
        ...
        >
          ...
          <MapContainer center={[43.6121, -116.3915]} zoom={12} scrollWheelZoom={true} style={{ height: "100%", minHeight: "100%" }}>
        <TileLayer
          attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
          url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
        />
          {boise.features && boise.features.map( feature => {
            return(
              <GeoJSON
                key={feature.id}
                data={feature} >
                <Tooltip permanent="true" direction="center" className="map-labels" style={{fontSize: "1.125rem"}}>
                  {feature.properties["CEILING"]}
                </Tooltip >
              </GeoJSON>
            )
          })}
          {mountainhome.features && mountainhome.features.map( feature => {
            return(
              <GeoJSON
                key={feature.id}
                data={feature} >
